...
About **$6k/month** of supporting services sits on top of the GPU line — database, vector store, knowledge graph, cache, load balancer, monitoring. Full breakdown in [PLATFORM_REPORT.md §9](PLATFORM_REPORT.md).
...
- **H200 over H100 costs ~15% more and buys the SLA back.** 141 GB HBM3e at 4.8 TB/s versus 80 GB at 3.35 TB/s gives ~1.43× decode — ~4.6 s end-to-end instead of ~6.4 s, and ~255 KV slots per replica instead of ~96. On a 3-year commit the delta is **$31,247/year**. H100 remains a costed fallback; the full comparison is in §11.2 of the report.
...
- **Self-hosting is ~4.3× more expensive than a commercial API at this volume** (~$72k/year). It is justified by the RL loop, data residency, and latency ownership — **not by cost**. Break-even is around 129,000 conversations/day. This is a product decision and should be made explicitly; see §10 of the report.
...
| [reference/SYSTEM_REQUIREMENTS.md](reference/SYSTEM_REQUIREMENTS.md) | Derivation and assumptions behind every figure — load model, the explicit mathematical model (§2.1), GPU sizing, the mathematical-model workload (§7), subsystem specs, and the single-GPU training assessment. **Optional reading.** |

---

## Confidence and caveats

These are **directional back-of-the-envelope estimates** — enough to scope a purchase order, not a substitute for benchmarking. Every assumption is listed in Appendix A of the report so it can be challenged and re-run. Four things should be settled before capital is committed:
...
4. **Specify the mathematical-model workload.** Beyond the LLM, the platform runs a multivariate pipeline — clustering, whitening/PCA, ICA, and **causal discovery (LiNGAM, DYNOTEARS, SF-Slinear)** — feeding **~156+ parametric time-series models** plus **recurrent forecasters (RNN, GRU, LSTM)**. It is off the request path, so it carries no SLA risk. It is **mostly CPU work with one small GPU stage**: the recurrent models are the only VRAM consumers, they are what the stated 16 GB VRAM refers to, and they are why the workload keeps its `g6.4xlarge` rather than moving to a CPU instance. Its dimensions are still unspecified, so it is carried as a $159/month placeholder. Thirteen inputs would close it, listed in [PLATFORM_REPORT.md §7](PLATFORM_REPORT.md) with the full derivation and per-method mathematics in [reference/SYSTEM_REQUIREMENTS.md §7](reference/SYSTEM_REQUIREMENTS.md). It is not a threat to the headline number — the spare capacity on the existing instance absorbs any plausible reading — but it should be sized rather than guessed.
