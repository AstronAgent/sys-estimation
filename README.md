...
4. **Specify the scheduled analytics pipeline.** Beyond the LLM, the platform runs a multivariate pipeline — clustering, whitening/PCA, ICA — feeding **~156+ parametric time-series models**. It is off the request path, so it carries no SLA risk, and it is **CPU work rather than GPU work**. But its dimensions are unspecified, so it is currently carried as a $159/month placeholder. Nine inputs would close it, listed in [reference/SYSTEM_REQUIREMENTS.md §6.2](reference/SYSTEM_REQUIREMENTS.md). It is not a threat to the headline number — the spare capacity on the existing instance absorbs any plausible reading — but it should be sized rather than guessed.
